...
int ledColor = 0; // global var to store color information 
int displayFlag = 0; // 0: display temperature 1: CAFE 2: word2
...
{ 
  ...
  /*************************************************/
    ...
    // receive 3 -> black out display
    if (incomingByte == 51){  
      displayFlag = 3;
    }
    // receive 4 -> change back to display temperature
    if (incomingByte == 52){  
      displayFlag = 0;
    }
    // receive 5 -> Light Red
  ...
  /*************************************************/
    ...
    if(temperatureUnitFlag) Cal_F_temp (Decimal, Temperature_H, Temperature_L, IsPositive);
...
void Dis_7SEG (int Decimal, byte High, byte Low, bool sign)
{
  ...
  //      Send7SEG (Digit,0x46);    /* Display "f" if there is more space on 7-SEG */
      }
      else{
      Send7SEG (Digit,NumberLookup[12]);
  //      Send7SEG (Digit,0x58);   /* Display "c" if there is more space on 7-SEG */
      }
      Digit--;
    }
  ...
  else if(displayFlag == 2){ /*display Penn*/
  ...
  else if(displayFlag == 3){ /*display black out*/
      byte Digit = 3;                 /* Number of 7-Segment digit */
